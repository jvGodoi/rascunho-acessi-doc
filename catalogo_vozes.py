...
CAMINHO_JSON_VOZES = Path(__file__).parent / "voices.json"

VOZES_ESTATICAS: List[Dict] = [
...
def _carregar_do_json() -> Optional[List[Dict]]:
    if CAMINHO_JSON_VOZES.exists():
        try:
            return json.loads(CAMINHO_JSON_VOZES.read_text(encoding="utf-8"))
        except Exception:
            return None
    return None

def carregar_vozes() -> List[Dict]:
    dados = _carregar_do_json()
    if isinstance(dados, list) and dados:
        return dados
    return VOZES_ESTATICAS

def vozes_por_idioma(codigo_idioma: str) -> List[Dict]:
    if not codigo_idioma:
        return []
    lang = codigo_idioma.split("-")[0].lower()  # 'pt'
    vozes = carregar_vozes()
    resultado = [v for v in vozes if v.get("locale", "").lower().startswith(f"{lang}-")]
    resultado.sort(key=lambda v: (v.get("locale", ""), v.get("displayName") or v.get("shortName")))
    return resultado

def voz_padrao_para_idioma(codigo_idioma: str, genero_preferido: Optional[str] = None) -> Optional[str]:
    candidatos = vozes_por_idioma(codigo_idioma)
    if not candidatos:
        return None
    if genero_preferido:
        for v in candidatos:
            if v.get("gender") == genero_preferido:
                return v["shortName"]
    return candidatos[0]["shortName"]

def como_voz_publica(v: Dict) -> Dict:
    return {
...
voices_for_lang = vozes_por_idioma
default_voice_for_lang = voz_padrao_para_idioma
as_public_voice = como_voz_publica
load_voices = carregar_vozes
